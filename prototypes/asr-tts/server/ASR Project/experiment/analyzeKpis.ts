...
function main() {
  ...
  const trials = [...groupBy(events, (event) => event.trialId).entries()]
    ...
    .map(([trialId, trialEvents]) => {
      ...
      return {
        ...
        stimulusVoice: first.stimulusVoice ?? null,
        stimulusFile: first.stimulusFile ?? null,
        confirmationStimulusFile: first.confirmationStimulusFile ?? null,
  ...
  for (const [, rows] of byVoice.entries()) {
    const first = rows[0];
    const latencies = rows
      .map((row: any) => row.endToEndLatencyMs)
      .filter((value: any) => typeof value === "number");

    voiceAggregateRows.push({
      condition: first.condition,
      prototype: first.prototype,
      environment: first.environment,
      stimulusVoice: first.stimulusVoice,
      totalTrials: rows.length,
      taskSuccessRate: rate(rows, "taskSuccess"),
      criticalExecutionErrorRate: rate(rows, "criticalExecutionError"),
      meanEntityAccuracy: mean(rows.map((row: any) => row.entityAccuracy)),
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(voiceAggregateRows, null, 2),
    "utf8"
  );
  writeCsv(path.join(RESULTS_DIR, "voice-aggregate-results.csv"), voiceAggregateRows, voiceAggregateColumns);
