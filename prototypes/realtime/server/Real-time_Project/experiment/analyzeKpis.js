...
function evaluateTrial(trialId, events, scenarioMap) {
  ...
  return {
    ...
    failedChecks: finalCheckResult.failedChecks,
    experimentId: start.experimentId ?? null,
    stimulusVoice: start.stimulusVoice ?? null,
    stimulusFile: start.stimulusFile ?? null,
...
function aggregateResultsByVoice(trialResults) {
  const withVoice = trialResults.filter((row) => row.stimulusVoice);
  const groups = new Map();

  for (const result of withVoice) {
    const key = `${result.condition}|${result.prototype}|${result.environment}|${result.stimulusVoice}`;
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(result);
  }
  ...
  for (const [key, rows] of groups.entries()) {
    const [condition, prototype, environment, stimulusVoice] = key.split("|");
    const entityValues = rows.map((row) => row.entityAccuracy).filter((value) => value !== null);
    const confirmationValues = rows.map((row) => row.confirmationCompliance).filter((value) => value !== null);
    const latencyValues = rows.map((row) => row.latencyMs).filter((value) => value !== null);
  ...
  return aggregates.sort((a, b) =>
    `${a.condition}|${a.stimulusVoice}`.localeCompare(`${b.condition}|${b.stimulusVoice}`)
  );
}
...
function main() {
  ...
  fs.writeFileSync(
    path.join(RESULTS_DIR, "voice-aggregate-results.json"),
    JSON.stringify(voiceAggregates, null, 2),
    "utf8"
  );
  ...
  writeCsv(path.join(RESULTS_DIR, "voice-aggregate-results.csv"), voiceAggregates);
